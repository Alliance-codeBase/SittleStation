...
/obj/machinery/power/energy_accumulator/rad_collector/examine(mob/user)
	...
	. += span_notice("[src]'s display states that it has stored <b>[DisplayJoules(get_stored_joules())]</b>, and is processing <b>[DisplayPower(processed_energy)]</b>.")
...
/obj/machinery/power/energy_accumulator/rad_collector/proc/hawking_pulse(atom/source, pulse_strength)
	if(loaded_tank && active && pulse_strength > RAD_COLLECTOR_THRESHOLD)
		stored_energy += DisplayJoules((pulse_strength - RAD_COLLECTOR_THRESHOLD) * RAD_COLLECTOR_COEFFICIENT * efficiency_multiplier)
